...
def sii_doublet(x, h1, h2, lmbda1, sig, c):
    return (h1/np.sqrt(2*np.pi)/sig)*np.exp(-(x-lmbda1)**2/2/sig**2) +            (h2/np.sqrt(2*np.pi)/sig)*np.exp(-(x-sii_wratio*lmbda1)**2/2/sig**2) + c
...
class Lines:
    ...
    def get_slice(self, arr_in: NDArray) -> NDArray:
        mask = (WLAX[1640] < WLAX) & (WLAX < WLAX[1740])
        return arr_in[mask]
